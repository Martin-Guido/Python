...
Original file is located at
    https://colab.research.google.com/drive/16OJ7TrGv-d4GdFVIJZ4y2KQrP908aGoz
...
#Para poder trabajar con los datos, declaramos las siguientes variables. 
#Las cuales luego van a almacenar los datos del archivo.
datos1=[]
datos2=[]
datos3=[]
datos4=[]
...
################################################################################
#####################################################
#Aca abrimos el archivo de nombre 2013.text con los datos del mes la temperatura
# el viento, la presion y la lluvia.
file=open('entrenamiento.text','r')
#definimos contenido tal que pueda almacenar los datos hasta 
#encontrar un tabulador '\t'
...
#  Así sucesivamente hasta que posiciones todos los separadores (en el caso de 
#que tu archivo tenga un número de columnas fijo)
...
#almacenamos los datos en listas y luego las convertimos en vectores para 
#trabajar con la biblioteca numpy econ el array.
#siendo los valores de almacenados en <<X>> los datos que usaremos en la capa 
#de entrada y <<Y>> el valor de la lluvia el cual deseamos hallar.
x=np.array(x)   
y=np.array(lluvia)
p=len(x.T)

#Modulador
#Hace el tratamiento a los datos para adaptarlos en un rango de 0 a 1 para
# luego utilizar la funcion de activacion.
def mod(x):
  #Se define la funcion elmo la cual al ser aplicada a cada valor hace una 
  #division entre la diferencia del maximo 
  #y el minimo de los vectores que presentan los valores de cada variable.
 elmo=lambda z,a:(z-min(a))/(max(a)-min(a))  
 c=[]
 #se realiza un tratamiento distinto para los valores de <<Y>> que para los de 
 #<<X>> ya que las dimenciones del vector <<Y>> son (57,)
 # la cual genera errores en el proceso.
 if len(x.T)==len(x):
  for j in range(len(x)):
      c.append(elmo(x[j],x))
 #este proceso separa los vectores columnas del array de <<X>> para poder hacer 
 #un tratamiento a cada tipo de variable por separado. 
...
##Ordenador
#se aplica un ordenador de datos porque se vio que optimizaba el trabajo de 
#aprendisaje de la red neuronal
#lo que hace es organizar los valores de precipitacion los del array<<Y>> 
#y los ordena de menor a mayor en un nuevo vector b y almacenos 
#los valores de y en c
def ordenador(x,y,il):
  if il==1:
    b=sorted(y,reverse=True)
  else:
    b=sorted(y,reverse=False)

  c=y
  h=mod(x)
  a=[]
  k=0
#Luego va recoriendo los valores del vector ordenado comparandolo con 
#cada elemento del vector sin ordenar
# para almacenar en a el vector ordenado de las variable 
#guardadas anteriormente en <<X>>
  for i in range(len(h)):
    ...
    for j in range(len(b)):
      if s==0 and b[i]==c[j]:
        ...
        k=k+1
        a.append(h[j])
   #se eliminan los elementos ya escritos para no repetir el mismo valor 
   #de <<X[j]>> si llega a repetirse el valor de <<Y[j]>> 
        h=np.delete(h, j,axis=0)
        b=np.delete(b, i,axis=0)
        c=np.delete(c, j,axis=0)
#al eliminarse los elementos la variable k
# vendria a ser una correccion a la posicion 
  return np.array(a)
...
def reordenador(x,y,il):
  
  

  if il==1:
    h=sorted(y,reverse=True)
  else:
    h=sorted(y,reverse=False)
  c=y
  a=[]
  k=0
  for i in range(len(h)):
...
l2_cost = (lambda yp,yr:np.linalg.norm((yp-yr)/len(yp)) , lambda yp,yr:(yp-yr))
...
il=0
def train(neural_net, x,y,il, l2_cost,ordenador,reordenador,mod, lr=0.8,train=True): 
 ...
 # y luego son ordenados de menor a menor
  if il==1:
    y=mod(np.array(sorted(y,reverse=True)))
  else :
    y=mod(np.array(sorted(y,reverse=False)))

 # Se guardan los valores de <<X>> ordenados con respecto al orden 
 # de menor a mayor de <<Y>> en <<Cin>> 
  Cin=ordenador(x,y,il)
 ...
 # Re ordena los datos calculados  
  result=reordenador(result,rec,il) 
  return result

"""Donde entrenamos y luego probamos la red neuronal"""
...
#(en el caso de que tu archivo tenga un número de columnas fijo)
   ...
   presion.append(valor4)
   lluvia.append((valor5))
   x1.append([mes[i],tmed[i],viento[i],presion[i]])
x1=np.array(x1)   
y1=np.array(lluvia)
y1=(mod(y1))
print('y',y1)
...
i=0
y=mod(y)
file=open('4-15-1.txt','w')
while i<20000:
  # Entrenemos a la red!
 

  pY =train(neural_net, x,y,il, l2_cost,ordenador,reordenador,mod, lr=0.1,train=True)
  pY1 = train(neural_net, x1,y1,il, l2_cost,ordenador,reordenador,mod, lr=0.01,train=False)
  ...
  poso=str(g*102)
  pred=str(pY1)
  file.write(ist + '\t' + pos+ '\t'+poso + '\t'+pred +'\n') 
  clear_output(wait=True)
  plt.show()
  plt.plot(range(len(loss)), loss)
  plt.savefig('name-4-600-1')
  plt.show()
  print('Error=',loss[-1])
  print('dy=',g*102)
